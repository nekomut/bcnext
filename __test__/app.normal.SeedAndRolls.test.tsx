...
import SeedAndRolls from '@/app/normal/SeedAndRolls';
...
describe('Normal SeedAndRolls', () => {
  ...
  test('initializes with localStorage seed if no URL parameter', () => {
    ...
    localStorageMock.setItem('normal_seed', '54321');
  ...
  test('updates URL when seed input changes', () => {
    ...
    expect(mockReplace).toHaveBeenCalledWith('/normal?seed=98765');
  });
  ...
  test('updates URL when rolls input changes', () => {
    ...
    // Should update the URL
    expect(mockReplace).toHaveBeenCalledWith('/normal?rolls=15');
  });
  ...
  test('saves seed to localStorage when it changes', () => {
    ...
    expect(localStorageMock.getItem('normal_seed')).toBe('13579');
  });
  ...
  test('uses default rolls value when input is less than 1', () => {
    ...
    expect(mockReplace).toHaveBeenCalledWith(`/normal?rolls=${DEFAULTS.rolls}`);
  });
  ...
  test('preserves other URL parameters when updating seed or rolls', () => {
    ...
    expect(mockReplace).toHaveBeenCalledWith('/normal?seed=54321&rolls=10&gatyasets=nyankogatya%2Cnyankoplus');
  });
});